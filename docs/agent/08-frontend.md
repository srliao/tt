...
Multi-select on `/tasks` is always available — selection state lives in `useSelection` (`features/tasks/use-selection.ts`, sessionStorage-backed) and `<BulkActionBar>` mounts whenever `selection.size > 0`. There's no mode toggle.
