...
describe('SHORTCUT_GROUPS', () => {
  it('exposes the three documented sections in order', () => {
    expect(SHORTCUT_GROUPS.map((g) => g.title)).toEqual(['Global', 'Navigate', 'On a task']);
  });

  it('includes every Phase 6 shortcut', () => {
...
describe('<ShortcutCheatsheet>', () => {
  it('renders all three group headings when opened', () => {
  ...
  it('renders the inline tag editor shortcut description', () => {
    render(<ShortcutCheatsheet />);
    openCheatsheet();
    expect(screen.getByText('Edit tags inline')).toBeTruthy();
    expect(screen.getByText('Extend selection range')).toBeTruthy();
  });
});
